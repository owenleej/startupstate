...
import { HeroPanel, HeroNudge } from "./HeroPanel";
...
const SECTIONS: Record<string, { color: string }> = {
  ...
  "Agriculture":      { color: "#84CC16" },
  "Manufacturing":    { color: "#78716C" },
};
...
export default function MapClient({ companies, isAdmin = false, isLoggedIn = false, ownedCompanies = [], investorProfile = null, followedCompanyIds: initialFollowedIds = new Set(), newsletterSubscribedIds = new Set() }: { companies: Company[]; isAdmin?: boolean; isLoggedIn?: boolean; ownedCompanies?: Company[]; investorProfile?: InvestorProfile | null; followedCompanyIds?: Set<number>; newsletterSubscribedIds?: Set<number> }) {
  ...
  function getPadding(sidebarOpen: boolean, panelOpen: boolean) {
    return {
      left: sidebarOpen ? 400 : heroOpen ? 390 : 0,
  ...
  return (
    <div className="fixed inset-0 bg-zinc-900">
      ...
      {/* ── Top bar ───────────────────────────────────────────────────────────── */}
      <header className="absolute top-0 left-0 right-0 z-10 flex items-center gap-2 p-2.5 pointer-events-none">
        <div className="pointer-events-auto flex items-center gap-2 bg-zinc-900/85 backdrop-blur-md border border-zinc-700/50 rounded-xl px-3 py-1.5 shadow-lg">
          <div className="w-10 h-10 rounded-md flex items-center justify-center flex-shrink-0" style={{backgroundColor: "#C1440E"}}>
            <svg className="w-7 h-7 text-white" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth={8} strokeLinejoin="round" strokeLinecap="round">
              <path d="M15 10 L65 10 L65 42 L85 42 L85 90 L15 90 Z" />
            </svg>
          </div>
          <span className="text-white font-semibold tracking-tight text-base">The Startup State</span>
        </div>
        ...
        <div className="pointer-events-auto flex-1 max-w-xs">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-zinc-900/85 backdrop-blur-md border border-zinc-700/50 text-white placeholder-zinc-500 rounded-xl pl-9 pr-4 py-1.5 text-sm shadow-lg focus:outline-none focus:border-blue-500/60 transition-colors"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
        ...
        <div className="pointer-events-auto flex items-center gap-1.5">
          <div className="bg-zinc-900/85 backdrop-blur-md border border-zinc-700/50 rounded-xl px-3 py-1.5 text-sm text-zinc-300 shadow-lg whitespace-nowrap">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white text-sm font-semibold rounded-xl px-3 py-1.5 shadow-lg transition-colors whitespace-nowrap"
          >
          ...
          {isLoggedIn && ownedCompanies.length === 0 && (
            <button
              ...
              className="bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white text-sm font-semibold rounded-xl px-3 py-1.5 shadow-lg transition-colors whitespace-nowrap"
            >
              + Add My Company
            </button>
          )}
          {isLoggedIn && !currentInvestorProfile && (
            <button
              ...
              className="flex items-center gap-1.5 text-sm font-semibold rounded-xl px-3 py-1.5 shadow-lg transition-colors whitespace-nowrap bg-zinc-900/85 backdrop-blur-md border border-zinc-700/50 text-zinc-300 hover:text-white hover:border-zinc-500"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {isLoggedIn && currentInvestorProfile && (
            <div className="flex items-center gap-1.5">
              ...
              <button
                onClick={() => setShowInvestorModal(true)}
                className="flex items-center gap-1.5 text-sm font-semibold rounded-xl px-3 py-1.5 shadow-lg transition-colors whitespace-nowrap bg-violet-600 hover:bg-violet-500 text-white"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                </svg>
              ...
              <button
                ...
                className={`flex items-center gap-1.5 text-sm font-semibold rounded-xl px-3 py-1.5 shadow-lg whitespace-nowrap ${
                  showInvestorSidebar
      ...
      {/* ── Filters ───────────────────────────────────────────────────────────── */}
      <div className={`absolute top-[72px] z-10 flex items-center gap-2 transition-all duration-300 ${showInvestorSidebar ? "left-[396px]" : heroOpen ? "left-[386px]" : "left-4"}`}>
        <FilterDropdown
          label="Sector"
          value={activeSection}
      ...
      {/* ── Hero panel (unauthenticated) ──────────────────────────────────────── */}
      {!isLoggedIn && !showInvestorSidebar && (
        <HeroPanel
      ...
      {/* ── Hero nudge (logged-in, no companies, no investor profile) ─────────── */}
      {isLoggedIn && ownedCompanies.length === 0 && !currentInvestorProfile && !showInvestorSidebar && (
        <HeroNudge
          onAddCompany={() => setShowAddCompany(true)}
          onSetupInvestor={() => setShowInvestorModal(true)}
        />
      )}
      ...
      {isLoggedIn && (
        <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
          {ownedCompanies.length > 0 && (
            <div className="relative" ref={myCompaniesDropdownRef}>
              <button
                ...
                className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white text-sm font-semibold rounded-xl px-3 py-1.5 shadow-lg transition-colors"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {isAdmin && (
            <a
              href="/admin"
              className="flex items-center gap-1.5 bg-zinc-900/85 backdrop-blur-md border border-zinc-700/50 hover:border-zinc-500 text-zinc-300 hover:text-white text-sm font-medium rounded-xl px-3 py-1.5 shadow-lg transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
