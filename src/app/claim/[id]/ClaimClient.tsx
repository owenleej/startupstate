...
import ResourceMatchModal from "@/components/ResourceMatchModal";
...
export default function ClaimClient({ company }: { company: Company }) {
  ...
  const [showResourceMatch, setShowResourceMatch] = useState(false);
  ...
  return (
            ...
            {isDone && (
              <div className="text-center">
                ...
                <div className="flex flex-col gap-3">
                  <button
                    onClick={() => setShowResourceMatch(true)}
                    className="btn-ai w-full flex items-center justify-center gap-2 px-5 py-2.5 text-sm"
                  >
                    ✦ Find programs that can help you → it&apos;s free
                  </button>
                  <a href="/" className="inline-block px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-sm font-semibold rounded-xl transition-colors">
                    Back to map
                  </a>
                </div>
              </div>
    ...
    {showResourceMatch && (
      <ResourceMatchModal
        ...
        isLoggedIn
        onClose={() => setShowResourceMatch(false)}
      />
    )}
    </>
  );
